    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for article in articles:
                ...
                review_button.wait_for(state='visible',timeout=1000)
                review_button.click()

                # 3. Wait for the review list to actually load
                # page.locator(".MyEned").first.wait_for(state='visible', timeout=10000)
                page.locator(".fontBodyMedium").first.wait_for(state='visible', timeout=1000)
                ...
                review_cards = page.locator('.fontBodyMedium').all()
                ...
                for review in review_cards:
                    try:
                        ...
                        # # Extract review text
                        # review_text = review.locator("div.MyEned").inner_text()
                        
                        # # Extract date with timeout handling
                        # try:
                        #     date = review.locator("xpath=.//span[contains(text(), 'ago')]").inner_text()
                        # except:
                        #     date = ""
        ...
        # 1. Use a strict, scoped locator for the review text
        # Adding a small timeout (e.g., 2 seconds) prevents the 30s hang
                        text_element = review.locator("div.MyEned")
                        
                        if text_element.count() > 0:
                            # Check for a "More" button and click it to expand the text
                            more_button = review.locator('button[aria-label*="See more"]')
                            if more_button.count() > 0 and more_button.is_visible():
                                more_button.click()
                            
                            review_text = text_element.inner_text(timeout=2000)
                        else:
                            review_text = ""

                        # 2. Extract rating safely
                        rating_elem = review.locator('span[role="img"][aria-label*="stars"]').first
                        rating = "0"
                        ...
                        # 3. Extract date (Relative dates like '2 weeks ago')
                        # Google uses the class 'rsqawe' or 'target text'
                        
                        date = page.locator("xpath=.//span[contains(text(), 'ago')]").first.inner_text()

                        if review_text:  # Only save if there's actual content
                            reviews_data.append({
                                'review': review_text,
                                'rating': rating,
                                'date': date
            ...
            if reviews_data:
                ...
                csv_filename = 'googlemaphotel102review.csv'
